...
function load() {
  if (window.location.host != "productforums.google.com" && window.location.host != "groups.google.com") {
    ...
    console.log("Not in productforums.google.com");
    return false;
  }
...
function jsoooon() {
  var trs = document.querySelectorAll("table[role=\"list\"] tr"); // Check
  var forum = trs[0].querySelector("a").href.split("/")[5]; // Check
  for (var i = 0; i < trs.length; i++) {
    var id = trs[i].querySelector("a").href.split("/")[6]; // Check
    threads.push(id); // Check
  }
  var json = {"forum": forum, "threads": threads};
  download(JSON.stringify(json), forum+"_threadlist.json", "application/json");
}
